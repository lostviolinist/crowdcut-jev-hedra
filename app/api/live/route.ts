...
export async function GET(request: Request) {
  try {
    ...
    const [scenesResult, commentsResult, ideasResult, classifiedResult] = await Promise.all([
      ...
      db.prepare("SELECT COUNT(*) AS count FROM live_comments WHERE round = ? AND state IN ('usable', 'off_topic')").bind(story.round).first<{ count: number }>(),
    ]);
