...
export async function POST(request: Request) {
  try {
    ...
    const inserted = synthetic
      ? await db.prepare("INSERT INTO live_comments (user_id, name, body, round, created_at) SELECT ?, ?, ?, ?, ? WHERE (SELECT COUNT(*) FROM live_comments WHERE round = ?) < 800 AND (SELECT COUNT(*) FROM live_comments WHERE round = ? AND user_id LIKE 'sim:%') < 500 AND NOT EXISTS (SELECT 1 FROM live_comments WHERE round = ? AND lower(trim(body)) = lower(trim(?))) RETURNING id")
        .bind(userId, name, body, story.round, now, story.round, story.round, story.round, body).first<{ id: number }>()
      : guest?.ipPrefix
        ? await db.prepare("INSERT INTO live_comments (user_id, name, body, round, created_at) SELECT ?, ?, ?, ?, ? WHERE (SELECT COUNT(*) FROM live_comments WHERE round = ?) < 800 AND (SELECT COUNT(*) FROM live_comments WHERE user_id = ? AND round = ?) < 3 AND (SELECT COUNT(*) FROM live_comments WHERE user_id LIKE ? AND round = ?) < 30 AND NOT EXISTS (SELECT 1 FROM live_comments WHERE user_id = ? AND created_at > ?) RETURNING id")
        ...
        : await db.prepare("INSERT INTO live_comments (user_id, name, body, round, created_at) SELECT ?, ?, ?, ?, ? WHERE (SELECT COUNT(*) FROM live_comments WHERE round = ?) < 800 AND (SELECT COUNT(*) FROM live_comments WHERE user_id = ? AND round = ?) < 3 AND NOT EXISTS (SELECT 1 FROM live_comments WHERE user_id = ? AND created_at > ?) RETURNING id")
