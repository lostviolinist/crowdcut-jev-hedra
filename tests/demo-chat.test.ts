...
test("each scene gets 75 distinct generated comments and handles", () => {
  for (let round = 1; round <= 100; round++) {
    const comments = Array.from({ length: 75 }, (_, offset) => makeLiveComment((round - 1) * 75 + offset, round));
...
test("live rounds introduce new visual directions instead of the same dialogue", () => {
  ...
  const secondRound = Array.from({ length: 75 }, (_, index) => makeLiveComment(index + 75, 2)).join(" ");
